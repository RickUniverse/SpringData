...
public interface PersonRepository
        extends JpaRepository<Person,Integer>, JpaSpecificationExecutor, PersonDao {
    //根据名称获取响应的名称
    ...
    //WHERE ENAMI IN(?,?,?) AND NAME LIKE ?%
    List<Person> getByEmailInAndNameStartingWith(List<String> emails, String name);

    //WHERE a.id > ?
    List<Person> getByAddress_IdGreaterThan(Integer id);

    /*使用Query注解*/
    @Query("SELECT p1 FROM Person p1 WHERE p1.id = (SELECT MAX(p2.id) FROM Person p2)")
    Person getMaxIdPerson();

    //使用占位符传入参数String name, String email需要一一对应
    @Query("SELECT p1 FROM Person p1 WHERE p1.name LIKE %?1% AND p1.email LIKE %?2%")
    List<Person> testQueryAnnotationParam1(String name, String email);

    //使用命名参数的方式
    @Query("SELECT p1 FROM Person p1 WHERE p1.name LIKE %:name% AND p1.email LIKE %:email%")
    List<Person> testQueryAnnotationParam2(@Param("email") String email, @Param("name") String name);

    //nativeQuery = true : 开启本地方法插叙
    @Query(value = "SELECT COUNT(*) FROM jpa_person", nativeQuery = true)
    Long testNative();


    //JPQL操作只可以进行查，改删操作，不能进行添加操作,必须加上@Modifying注解指明修改或删除操作
    //且必须在事务下进行操作
    @Modifying
    @Query("UPDATE Person p set p.email = :email WHERE p.id = :id")
    Integer testUpdatePersonEmail(@Param("email") String email, @Param("id") Integer id);
